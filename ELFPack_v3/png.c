...
__arm IMGHDR *find_png_in_cache(char *png_name)
{
  ...
  while((pl=pl->next))
  {
    // Call Swee too inhibitory to these cycles: D
    if (!strcmp(pl->pngname,png_name))
    {
      //Found, transfer to the start and exit
      if (pl_prev)
      {
	//Only if not in the beginning
	pl_prev->next=pl->next; //removed from the site found
	pl->next=(PNGLIST *)(pngtop.pltop); //next - the whole list
	pngtop.pltop=pl; //And the first in the list - more information!
      }
      UnlockSched();
      return(pl->img);
    }
    pl_prev=pl; //Current finished - now the previous
  }
  UnlockSched();
  return (0);
}
...
__arm IMGHDR* PatchGetPIT(unsigned int pic)
{
  ...
  {
    //We are looking for in the list of dynamic icons
    {
      DYNPNGICONLIST *dynp;
      LockSched();
    ...
    if ((pngtop.bitmap) && (pic<20000))
    {
      ...
      if ((i=*bp)&mask80)  // There is an entry in the bitmap
      {
        if (i&mask40)  
        {
          char *next=strcpy_tolow(fname,IMAGE_FOLDER); // The picture is kind of like to drive
	  //*fname=DEFAULT_DISK_N+'0';
        ...
        else return(0);                                // Pictures do not - leave
      }
      else 
      {
	LockSched();
	...
	//*fname=DEFAULT_DISK_N+'0';
	  ...
	  *bp|=mask40;
	  UnlockSched();
        ...
        img=create_imghdr(fname,0);                 // We try to create
        if (img)
        {
	  LockSched();
	  *bp|=mask40;
  ...
  //They found nothing, and now try to add
  
  //if (!img) return (0); //Nothing to add
  cur=malloc(sizeof(PNGLIST)); //Create a list item
  cur->pngname=malloc(strlen(fname)+1);
  strcpy(cur->pngname,fname);
  cur->img=img;
  i=0; //This is the number of items in the list
  LockSched();
  cur->next=(PNGLIST *)(pngtop.pltop); // next - the whole list
  pngtop.pltop=cur; //First on the list - a new element
  //Trim out now
  pl=(PNGLIST *)(&(pngtop.pltop));
  do
  {
    ...
    if (!pl)
    {
      //Ended items beforeше
      UnlockSched();
      return(cur->img);
    }
    i++;
  }
  while(i<=CACHE_PNG); //While the number of elements is less than the allowable
  pl_prev->next=NULL; //gather the list
  UnlockSched();
  //The rest can be unlocked with multitasking
  do
  {
    //Remove the current
    if (pl->img)
  ...
  while(pl); //While there are elements, we release them
  return(cur->img);
}
