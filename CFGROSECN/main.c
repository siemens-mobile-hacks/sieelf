...
void on_utf8ec(USR_MENU_ITEM *item)
{
  if (item->type==0)
  {
    ...
    case 0:
      ...
      Ascii2WS(codemap,item->ws,"СЎФсДїВј",0);
      break;
    case 1:
      //wsprintf(item->ws,_percent_t,"SelectFile");
      Ascii2WS(codemap,item->ws,"СЎФсОДјю",0);
      break;
    }
  }
  if (item->type==1)
...
void maincsm_oncreate(CSM_RAM *data)
{
  ...
  if (find_cfg)
  {
    //UpdateCSMname("СЎФсЕдЦГОДјюSelect BCFG");
    UpdateCSMname("ЕдЦГОДјю");
...
int create_ed(CFG_HDR *need_to_focus)
{
  ...
  while(n>=sizeof(CFG_HDR))
  {
    ...
    case CFG_UINT:
      ...
      {
      L_ERRCONSTR:
        Ascii2WS(codemap,ews,"ЕдЦГТвНвЅбКш!!!",0);
    ...
    case CFG_CBOX:
      ...
      if (i>=hp->max)
      {
        Ascii2WS(codemap,ews,"»µЛчТэФЪБР±нЦР!!!",0);
    ...
    default:      
      sprintf(ERROR,"І»Ц§іЦПоДї:%d",hp->type);
