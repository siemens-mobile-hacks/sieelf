...
extern void fprintf(char *filename, char *format, ...);
...
int VKP_VoidStringsCleaner(char *vkp_body, int vkp_size, int *vkp_new_size)
{
  ...
  VKP_STRING *vs = (VKP_STRING *)malloc((nstrings + 1)*sizeof(VKP_STRING));
  ...
  if ((vs[nstrings - 1].offset + vs[nstrings - 1].size + 1) !=  vkp_size)
   {
      vs[nstrings].offset = vs[nstrings - 1].offset + vs[nstrings - 1].size + 1;
      vs[nstrings].size   = vkp_size  -  vs[nstrings].offset;
      nstrings++;
   }
...
int VKP_WrongSymbolsDetecter(char *vkp_body, int vkp_size)
{
  ...
  if ((vs[nstrings - 1].offset + vs[nstrings - 1].size + 1) !=  vkp_size)
   {
      vs[nstrings].offset = vs[nstrings - 1].offset + vs[nstrings - 1].size + 1;
      vs[nstrings].size   = vkp_size  -  vs[nstrings].offset + 1; // <<< +1
      nstrings++;
   }
