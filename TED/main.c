...
char t_bm1[]="书签1  000000000";
char t_bm2[]="书签2  000000000";
char t_bm3[]="书签3  000000000";
char t_bm4[]="书签4  000000000";
...
int u_disk; //蔫耜钼 箨噻囹咫?忮瘐礤泐 耱尻?(?犭铌圊 镱 STKSZ50)

int d_disk; //蔫耜钼 箨噻囹咫?龛骓邈?耱尻?(?犭铌圊 镱 STKSZ50)

unsigned int usp; //雨噻囹咫?磬 忮瘐龛?耱尻
unsigned int dsp; //雨噻囹咫?磬 龛骓栝 耱尻
...
{
  if (Q_DiskError&(1<<mode)) return; //亦觐?耦钺龛?箧?铗钺疣玷腓
  LockSched();
  Q_DiskError|=(1<<mode);
  switch(mode)
  {
  case 4:
    ShowMSG(1,(int)"无法新建或写入文件!");
    break;
  case 3:
    ShowMSG(1,(int)"无法打开缓存文件!");
    break;
  case 2:
    ShowMSG(1,(int)"无法在缓存文件内跳转!");
    break;
  case 1:
    ShowMSG(1,(int)"无法写入缓存文件!");
    break;
  case 0:
    ShowMSG(1,(int)"无法读取缓存文件!");
    break;
  }
  UnlockSched();
}
...
void DrawChar(int c,int x,int y)
{
  ...
  case 4:
    s=font+(c<<3);
    //吾疣徉螓忄屐 纛眚 疣珈屦铎 4
    d=myscr+(y*(8*SCR_MODULO)+(x>>1)); //务眍忭铋 疣?
    ms=0xF0;
    md=0x0F;
  ...
  case 6:
    s=font+(c<<3);
    //吾疣徉螓忄屐 纛眚 疣珈屦铎 6
    d=myscr+(y*(8*SCR_MODULO)+((x>>2)*3)); //0E:160C - 务眍忭铋 疣?
    switch(x&3)
    {
    case 0:
      do
  ...
  case 8:
    s=font+(c<<3);
    //吾疣徉螓忄屐 纛眚 疣珈屦铎 8
    d=myscr+(y*(8*SCR_MODULO)+x); //0E:160C - 务眍忭铋 疣?
    do
    {
      *d=*s++;
      d+=SCR_MODULO;
  ...
  case 14:
    ...
    s=font+(c*14);
    //吾疣徉螓忄屐 纛眚 疣珈屦铎 8
    d=myscr+(y*(14*SCR_MODULO)+x); //0E:160C - 务眍忭铋 疣?
    do
    {
      *d=*s++;
      d+=SCR_MODULO;
  ...
  case 16:
    ...
    s=font+(c*16);
    //吾疣徉螓忄屐 纛眚 疣珈屦铎 16
    d=myscr+(y*(16*SCR_MODULO)+x); //0E:160C - 务眍忭铋 疣?
    do
    {
      *d=*s++;
      d+=SCR_MODULO;
...
void drawStkStr(char *p, unsigned int y, unsigned int vp, int ep)
{
  ...
  while(i<max_x);
  if (ep>=0) while((c=*p++)) editline[ep++]=c; //念徼忄屐 铖蜞蝾?耱痤觇
}
...
void drawFrmStkStr(char *p, unsigned int y, unsigned int vp, int ep)
{
  ...
  if (i>max_x)
    i=0; //湾 漕徉怆屐 镳钺咫? 耱痤赅 潆桧礤? 麇?疣?
  else
    i=max_x-i; //义镥瘘 ?i - 钺?觐腓麇耱忸 漕徉怆屐 镳钺咫钼
  spcadd=0;
  spcsum=0;
  if (spcs)
  {
...
//襄疱眍?桤 忮瘐礤泐 耱尻??龛骓栝 (磬 耱痤牦 忖屦?
extern void move_up(void);
//襄疱眍?桤 龛骓邈?耱尻??忮瘐龛?(磬 耱痤牦 忭桤)
...
void DoDiskAccess(unsigned int flag)
{
  ...
  if ((f=stk_fhandle)==-1)
  {
    stk_fhandle=f=fopen(stkfile,A_ReadWrite+A_BIN,P_READ+P_WRITE,&ul); //脏殡 忮瘐礤泐 耱尻?
...
//青镨襦螯 桉蝾痂? 恹琨忄弪? ?觐眚尻耱?MMC_Filesystem
...
// 杨狁蜮屙眍 疱溧牝铕
//------------------------------------------
void GotoLine(void) //襄疱躅?磬 耱痤牦 seek_to_line, 恹琨忄弪? ?觐眚尻耱?MMC_Filesystem
{
  // unsigned int l;
  // unsigned int p;
  unsigned int ul;
  ...
  for(;;)
  {
    disk_access=0; //青觐眵桦?滂耜钼 铒屦圉梃
    CheckDiskAccess();
    if (disk_access) DoDiskAccess(0);
    if (seek_to_line==curline) break; //橡桫腓
    if (seek_to_line<curline)
    {
      //消蜞屐? 桎蜩 忖屦?
      if (!curline) break; //磬鬣腩!
      if (curline==viewline) viewline--; //篷腓 镥疴? 耱痤赅 疣磬
      curline--;
      move_up();
    }
    else
...
void DrawScreen(void)
{
  ...
  if (disk_access)
  {
    ...
    if (draw_mode==2)
    {
    	sprintf(upinfo_chr,"转到行 %u...",curline);
    ...
    if (disk_access==FIRSTLOAD)
    {
      ...
      sprintf(upinfo_chr,"行 %u...",curline);
      utf8_2ws(upinfo_ws, upinfo_chr, strlen(upinfo_chr));
    L_W1:
  ...
  else
  {
    ...
    case 1:
      ...
      y=curline-viewline;
      p=usp;
      if (dsp!=STKSZ) c=dstk[dsp]; else c=0; //襄疴 耔焘铍 蝈牦?耱痤觇 - 羼腓 狍赈?- 纛痨囹桊箦?
      while(y)
      {
	//篷螯 耱痤觇 恹 蝈牦?
	p=bl_us(p);
	...
	else
	{
	  unsigned int p1;
	  p1=bl_ds(p);
	  if (p1!=STKSZ) c=dstk[p1]; else c=0; //项耠. 耱痤赅 礤 疣聒桊弪?
	  if ((c>' ')&&(!(HISTORY.fmt&0x80))&&(cursor_off))
      ...
      {
	//需耋屐 耜痤腚-徉?
	char *d=myscr+((scr_w-1)>>3); //项耠邃龛?徉轵
	if (total_line)
	{
...
{
  {NULL,(int)"插入行"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"删除行"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"断开行"     ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"连接行"     ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"插入时间"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"插入日期"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"粘贴"          ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"清空剪贴板",LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2}
};

HEADER_DESC edmenu_HDR={0,0,0,0,icon,(int)"编辑...",LGP_NULL};
...
void ed_inp_redraw(void *data)
{
  ...
  sz=bl_ds(sz); //腮屐 磬鬣腩 耠. 耱痤觇
  sz-=p+1; //马钼?漕徉怆屐? 耱痤赅
  strcpy(dstk+(dsp=sz),editline); //念徉怆屐
  draw_mode=1;
  DrawScreen();
}
...
{
  static SOFTKEY_DESC sk={0x0FFF,0x0000,(int)"菜单"};
...
HEADER_DESC ed_inp_hdr={0,0,0,0,icon,(int)"编辑文本",LGP_NULL};
...
void CreateEditDialog(void)
{
  ...
  editmode=0;
  cursor_off=0;
  while ((curline-viewline)>=max_y_emode) viewline++; //篷腓 龛驽 麇?16 耱痤?- 镥疱戾屐?
...
{
  if (disk_access) return(0); //篷腓 滂耜钼 铒屦圉梃
  if ((draw_mode>1)&&(draw_mode!=4)) return(0); //篷腓 妁?痂耋屐
  ...
  if ((msg->gbsmsg->msg==KEY_DOWN)||(msg->gbsmsg->msg==LONG_PRESS))
  {
    ...
    case RIGHT_SOFT:
      //  L_EXIT:
      return(1); //橡铊聃钿栩 恹珙?GeneralFunc 潆 蝈? GUI -> 玎牮桢 GUI
    case GREEN_BUTTON:
      //    L_EDIT:
      CreateEditDialog();
      return(0);
	...
	LineUp();
      ...
      {
      unsigned int i=strlen(dstk+dsp); //碾桧?蝈? 耱痤觇
      char far *s;
      s=EX_heap_malloc_with_lock(i+2); //?? 漕徉怆屐 0 ?0d
      if (s)
      {
      strcpy(s,dstk+dsp);
      s[i]=0x0D;
...
int sf_inp_onkey(GUI *data, GUI_MSG *msg)
{
  if (msg->keys==0xFFF)
  {
    ...
    UpdateCSMname();
    HISTORY.fmt=0; //灭箸桁 邈?蝈镥瘘 赅?DirectLoad
...
HEADER_DESC sf_inp_hdr={0,0,0,0,icon,(int)"保存为:",LGP_NULL};
...
{
  font_size=sz;
  if (disk_access==FIRSTLOAD) //橡桫腓 桤 镥疴铋 玎沭箸觇
  {
    ShowMSG(1,(int)"字体已变换!");
    return;
  }
  disk_access=FIRSTLOAD;
  draw_mode=255; //蓐疣?镳桠弪耱忤
  SUBPROC((void *)loadfont,1);
  GeneralFuncF1(1); //青牮噱?戾睨
}
...
{
  {NULL,(int)"4号字体" ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"6号字体" ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"8号字体" ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"14号字体",LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"16号字体",LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"直接载入"   ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"DOS格式"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"WIN格式"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"开/关Padding",LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"保存为..."    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2}
};

HEADER_DESC loadmenu_HDR={0,0,0,0,icon,(int)"主菜单...",LGP_NULL};
...
{
  static SOFTKEY_DESC sk={0x0FFF,0x0000,(int)"转到..."};
  EDITCONTROL ec;
  if (cmd==7)
  {
    SetSoftKey(data,&sk,SET_SOFT_KEY_N);
...
HEADER_DESC gl_inp_hdr_percent={0,0,0,0,icon,(int)"转到百分之:",LGP_NULL};
HEADER_DESC gl_inp_hdr_line={0,0,0,0,icon,(int)"转到行:",LGP_NULL};
...
{
  GeneralFuncF1(1);
  goto_line_inp(0); //橡桤磬?镥疱躅溧 镱 耱痤赍
}

void goto_percent(void)
{
  GeneralFuncF1(1);
  goto_line_inp(1); //橡桤磬?镥疱躅溧 镱 镳铞屙蜞?
}
...
{
  {NULL,(int)"顶部"      ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"行"        ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"百分之?"   ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"底部"      ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
  {NULL,(int)"上一次"    ,LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
...
HEADER_DESC gotomenu_HDR={0,0,0,0,icon,(int)"转到...",LGP_NULL};
...
HEADER_DESC bookmenu_HDR={0,0,0,0,icon,(int)"设置书签...",LGP_NULL};
...
MENUITEM_DESC softmenu_ITEMS[3]=
{
  ...
  {NULL,(int)"设置书签...",LGP_NULL,0,NULL,MENU_FLAG3,MENU_FLAG2},
...
HEADER_DESC softmenu_HDR={0,0,0,0,icon,(int)"TED菜单",LGP_NULL};
...
unsigned int FL_loader(int fin, unsigned int p)
{
  ...
  if (p>=STKSZ50) //篷腓 箨噻囹咫?镥疱?玎 沭囗桷?镱腩忤睇 狍翦疣
  {
    memcpy(dstk,dstk+STKSZ50,STKSZ50); //暑镨痼屐 觐礤?狍翦疣 ?磬鬣腩
    p-=STKSZ50; //皖恹?箨噻囹咫?
    i=fread(fin,dstk+STKSZ50,STKSZ50,&ul); //央邃簋?镱?狍翦疣
    if (i<STKSZ50) dstk[i+STKSZ50]=0; //体睃 镱腩忤睇 - 爨痍屦 觐眦?羿殡?
  }
  return(p);
}

//青镨顸 忮瘐礤泐 耱尻?镳?磬鬣朦眍?玎沭箸赍
...
void FirstLoadFile(unsigned int fmt)
{
  ...
  u_disk=-1; //蔫耜钼 箨噻囹咫?忮瘐礤泐 耱尻?
  d_disk=-1; //蔫耜钼 箨噻囹咫?龛骓邈?耱尻?
  zeromem(ubat,sizeof(ubat)); //橡铟棂噱?蜞犭桷?犭铌钼 忮瘐礤泐 耱尻?
  zeromem(dbat,sizeof(dbat)); //橡铟棂噱?蜞犭桷?犭铌钼 龛骓邈?耱尻?
  zeromem(bfree,sizeof(bfree)); //橡铟棂噱?蜞犭桷?玎?螓?犭铌钼
  usp=0; //雨噻囹咫?磬 忮瘐龛?耱尻
  dsp=STKSZ; //雨噻囹咫?磬 龛骓栝 耱尻
  viewpos=0;
  curpos=0;
  curline=0;
  text_changed=0;
  ...
  if (fmt==0xFFFFFFFF)
  {
    ...
    {
      win_dos_koi=0xFF; //湾桤忮耱睇?
      switch(AUTOF_FONT) //仞梏?
      {
      ...
      zeromem(&HISTORY.line,4*6); //埋?磬 襦祛?忮瘐?
      HISTORY.cursor_off=cursor_off=1; //蔓觌栩?牦瘃铕
      HISTORY.total=1;
      //      HISTORY.fmt=0;
      HISTORY.fmt=255; //襄疴 玎矬耜!!!!
      if (ENA_AUTOF)
      {
      ...
      {
        LockSched();
        loadmenu_id=DrawLoadMenu(); //物疱溴?屐, 赅?沭箸栩?麇疱?戾睨
        UnlockSched();
        return;
      }
    }
  ...
  //暑礅屦蜩痼屐 怦?耱痤觇 ?忮瘐龛?耱尻
  fs=fopen(stkfile,A_Create+A_ReadWrite+A_BIN,P_READ+P_WRITE,&ul); //脏殡 忮瘐礤泐 耱尻?
  ...
  fclose(fs,&ul);
  total_line=viewline=curline; //袜躅滂祚 ?镱耠邃礤?耱痤赍
  HISTORY.font=font_size;
  HISTORY.fmt=fmt;
  ...
  {
    seek_to_line=HISTORY.line; //橡祛?镥疱躅?
  }
  else
  {
    if (HISTORY.total)
      seek_to_line=(total_line*HISTORY.line)/HISTORY.total; //项 铗眍龛?
    else
      seek_to_line=0;
...
int maincsm_onmessage(CSM_RAM *data, GBS_MSG *msg)
{
  ...
  if (msg->msg==MSG_GUI_DESTROYED)
  {
	...
	return(1);
      ...
      if (disk_access)
      {
	DoDiskAccess(1); //殷?牮桠铖螯, 镳桧蝾 麇疱?SUBPROC
      }
      CreateEditDialog();
    }
  }
...
int main(char *exename, char *fname)
{
  ...
  if ((ustk=malloc(STKSZ)))
  {
    ...
    mfree(ustk);
    LockSched();
    ShowMSG(1,(int)"磁盘空间不足!");
    UnlockSched();
    SUBPROC((void *)Killer2);
    return 0;
  }
  LockSched();
  ShowMSG(1,(int)"无法申请USTK!");
  UnlockSched();
  SUBPROC((void *)Killer2);
  return 0;
}
