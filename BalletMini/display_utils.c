...
int LineDown(VIEWDATA *vd)
{
  LINECACHE lc;
  unsigned int h;
  unsigned int pos=vd->view_pos; // pos - ýòî ïîñëåäíèé ñèìâîë â ñòðîêå
...
static const HEADER_DESC input_box_hdr={0,0,0,0,NULL,(int)"ÊäÈë",LGP_NULL};

static void input_box_ghook(GUI *data, int cmd)
{
  static SOFTKEY_DESC sk={0x0FFF,0x0000,(int)"È·¶¨"};
  if (cmd==0x0A)
  {
    DisableIDLETMR();
  }
...
HEADER_DESC sel_HDR={0,0,0,0,NULL,(int)"Ñ¡Ôñ",LGP_NULL};
