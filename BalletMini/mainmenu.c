...
static const HEADER_DESC add_bookmark_hdr={0,0,0,0,NULL,(int)"书签...",LGP_NULL};
...
int CreateAddBookmark(GUI *data)
{
  ...
  gb2ws(ews,"连接:",strlen("连接:"));
  ConstructEditControl(&ec,ECT_HEADER,ECF_APPEND_EOL,ews,wslen(ews));
  AddEditControlToEditQend(eq,&ec,ma); 

  if(data && flag)
    gb2ws(ews,view_url+2,strlen(view_url+2));
  else
    if ((main_csm=(MAIN_CSM *)FindCSMbyID(maincsm_id)))
    {
      p=FindGUIbyId(main_csm->view_id,NULL);
...
MENUITEM_DESC options_menu_ITEMS[OPTIONS_ITEMS_N]=
{
  ...
  {NULL,(int)"更改",            LGP_NULL, 0, NULL, MENU_FLAG3, MENU_FLAG2}, //1
...
{
  static SOFTKEY_DESC sk={0x0FFF,0x0000,(int)"搜索"};
...
static const HEADER_DESC input_url_hdr={0,0,0,0,NULL,(int)"地址",LGP_NULL};
...
static HEADER_DESC main_menuhdr={0,0,0,0,NULL,(int)"菜单",LGP_NULL};
