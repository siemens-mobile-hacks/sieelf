...
static void BuildGUI(uword mode,char *name,uword dlg);
//关闭有的菜单
...
static void DrawAlarm(void){     
  ...
  //显示日期
   ...
   if(AlrmInx%2){
     ...
     if(ALRM_VENA)SetVibration(ALRM_VIB);
   }else{
     SetIllumination(0,1,5,0);
     if(ALRM_VENA)SetVibration(0);
   }
...
static void BuildGUI(uword mode,char *name,uword dlg){  
  ...
  TASK_ID=CreateCSM(&TASK_CSM.CSM,DUMMY,dlg);  
  UnlockSched();
}
...
static int DaemonOnMessage(CSM_RAM* data,GBS_MSG* msg){   
 ...
 return(OICSMOnMessage(data, msg));
}  

static void DaemonOnCreate(void){
